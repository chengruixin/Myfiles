<?php 
	...
	if( empty($input_name) || empty($input_password) || empty($input_age) || empty($input_sex) || empty($input_email) || empty($input_phone_number) || empty($input_position) ){
 	...
 	else if($input_sex!="Female" && $input_sex!="Male"){
		header("Location: ../admin_add_users.php?add=sex");
		exit();
	}
	...
	else if($input_position!="Employee" && $input_position!="Admin" && $input_position!="Instructor"){
		header("Location: ../admin_add_users.php?add=position");
		exit();
	}
